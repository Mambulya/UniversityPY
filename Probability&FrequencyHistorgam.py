...
fig1, diagram1 = plt.subplots(figsize =(13, 6))
plt.hist(data,intervals, color="pink", ec="red", density=True)
plt.xticks(intervals)
plt.xlabel("Values")
...
б ~ {}
Dx = б^2 ~ {}
""".format( MAX, MIN, k, np.round(step, 3), np.round(interval1, 3), np.round(intervalK, 3), np.round(Ex, 3), np.round(б, 3), np.round(Dx, 3)))
